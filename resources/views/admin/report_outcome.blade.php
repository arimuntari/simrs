		...
		<fieldset>
			...
			<div class="row ">
				<div class="col-md-4">
					<div class="row">
						<div class="col-xs-6">
							<a target="_blank" href="{{ route('print.outcome')  }}?date_start={{ $date_start }}&date_end={{ $date_end}}" class="btn btn-success btn-block"> <i class="fa fa-print"></i> Print 
							</a>
						</div>
						<div class="col-xs-6">
							<a href="{{ route('report.outcome')  }}" class="btn btn-default btn-block"> <i class="fa fa-sync"></i> Refresh 
							</a>
						</div> 
					</div> 
				</div>
				...
				<div class="col-md-7">
					<div class="row">
						<form method="get" class="form-horizontal" action="{{  route('report.outcome')  }}">
							...
							<div class="col-xs-3 col-md-3  col-lg-2" style="font-size:13px;">
								<p class="form-control-static pull-right"><b>Total Data :  {{ $outcomes->total() }}</b></p>
							</div>
						</form>
					</div>
				</div>
			...
			<div class="table table-responsive mt-3">
				<table class="table table-bordered">
					...
					<tbody>
						<?php  $no = 1*$outcomes->currentPage() ;$totaloutcome = 0; ?>
						@foreach($outcomes as $outcome)
						<?php  $totaloutcome +=$outcome->price_total; ?>
						<tr>
							<td>{{ $no }}. </td>
							<td>{!! $outcome->code !!}</td>
							<td>{{ Helper::formatIndo($outcome->purchase_date) }}</td>
