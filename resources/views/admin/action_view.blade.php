	  ...
	  <legend><h4> {!! $icon." ".$title !!}</h4></legend>
        <div class="row ">
          <div class="col-md-4">
            <div class="row">
              <div class="col-xs-6">
                <a href="{{ route('action.create')  }}" class="btn btn-info btn-block"> <i class="fa fa-plus"></i> Tambah 
                </a>
              </div>
              <div class="col-xs-6">
                 <a href="{{ route('action.index')  }}" class="btn btn-default btn-block"> <i class="fa fa-sync"></i> Refresh 
                 </a>
              </div> 
            </div> 
          </div>
          ...
          <div class="col-md-7">
            <div class="row">
              <form method="get" class="form-horizontal" action="{{  route('action.index')  }}">
		          ...
		          <div class="col-xs-3 col-md-3  col-lg-2" style="font-size:13px;">
		          	<p class="form-control-static pull-right"><b>Total Data :  {{ $actions->total() }}</b></p>
		          </div>
              </form>
            </div>
          </div>
	  ...
	  <div class="table table-responsive mt-3">
		<table class="table table-bordered">
          ...
          <th width="150px">Biaya</th>
			...
			<tbody>
        <?php  $no = 1*$actions->currentPage() ; ?>
        @foreach($actions as $action)
				<tr>
					<td>{{ $no }}. </td>
					<td>{!! Helper::highLight($action->name, $key) !!}</td>
          <td align="right">@rupiah($action->price) </td>
          <td style="word-wrap: nowrap" align="center">
            <form id="delform" action="{{ route('action.destroy', $action->id ) }}" method="POST"> 
                {{ method_field('DELETE') }}
               @csrf
